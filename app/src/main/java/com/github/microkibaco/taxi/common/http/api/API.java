package com.github.microkibaco.taxi.common.http.api;

/**
 * https://www.easyapi.com/api/?documentId=8067&themeId=&categoryId=13213
 */
public class API {

    public static final String TEST_GET = "/get?uid=${uid}";
    public static final String TEST_POST = "/post";
    private static final String KEY = "/f34e28da5816433d/";

    // 获取验证码
    public static final String GET_SMS_CODE = KEY +
            "getMsgCode?phone=${phone}";
    // 校验验证码
    public static final String CHECK_SMS_CODE =
            KEY + "checkMsgCode?phone=${phone}&code=${code}";
    // 注册
    public static final String REGISTER = KEY + "register";
    // 登录
    public static final String LOGIN = KEY + "auth";
    // token 登录
    public static final String LOGIN_BY_TOKEN = KEY + "login";
    //获取附近司机
    public static final String GET_NEAR_DRIVERS = KEY + "getNearDrivers?latitude=${latitude}&longitude=${longitude}";
    // 上报当前位置
    public static final String UPLOAD_LOCATION = KEY + "updateUserLocation";
    //  呼叫司机
    public static final String CALL_DRIVER = KEY + "callDriver";

    // 取消订单
    public static final String CANCEL_ORDER = KEY + "cancelOrder";
    // 支付成功
    public static final String PAY = KEY + "paySuc";
    // 获取当前正在处理中的订单
    public static final String GET_PROCESSING_ORDER = KEY + "getProcessingOrder";

    // 检查用户是否存在
    public static String CHECK_USER_EXIST =
            KEY + "isUserExist?phone=${phone}";


    /**
     * 配置域名信息
     */
    public static class Config {

        private static final String TEST_DOMAIN = "http://cloud.bmob.cn/";
        private static final String RELEASE_DOMAIN = "http://cloud.bmob.cn/";
        private static final String TEST_APP_ID = "e90928398db0130b0d6d21da7bde357e";
        private static final String RELEASE_APP_ID = "e90928398db0130b0d6d21da7bde357e";
        private static final String TEST_APP_KEY = "514d8f8a2371bdf1566033f6664a24d2";
        private static final String RELEASE_APP_KEY = "514d8f8a2371bdf1566033f6664a24d2";
        private static String appId = TEST_APP_ID;
        private static String appKey = TEST_APP_KEY;
        private static String domain = TEST_DOMAIN;

        public static void setDebug(boolean debug) {
            domain = debug ? TEST_DOMAIN : RELEASE_DOMAIN;
            appId = debug ? TEST_APP_ID : RELEASE_APP_ID;
            appKey = debug ? TEST_APP_KEY : RELEASE_APP_KEY;
        }

        public static String getDomain() {
            return domain;
        }

        public static String getAppId() {
            return appId;
        }

        public static String getAppKey() {
            return appKey;
        }

    }

}
